...
bun test > /tmp/release_units.txt 2>&1
echo "EXIT=$?"
tail -50 /tmp/release_units.txt
grep -E '(fail\)|✗|error:' /tmp/release_units.txt | head -30
```
...
The pipe form silently breaks test failure ownership triage and
the test verification gate because:
...
once**. Keep these in sync and run the version-consistency audit below before
pushing. The canonical list lives here for release preparation and upgrades.
...
**Before allocating a release version**, fetch the target branch and check its
VERSION, the branch's existing bump, and pending releases. Reuse a consistent
existing bump; stop on unexplained drift. If another release claims the version,
allocate a new version and regenerate all derived files before publishing.
...
- Stop before pushing when any version location disagrees; resolve the drift
...
Run this audit after every merge resolution or version edit and before pushing.
...
In Conductor workspaces, run the branch-name check before pushing or creating a PR.
...
and the community-PR-wave process. Use that guide with the available native tools. Every
...
audit), the Conductor branch=workspace rule (above), the release documentation check (below),
the Privacy + Responsible-disclosure rules (below), and the PR-title-version-first rule
(below).

## Post-ship requirements (MANDATORY)

Before release closeout, inspect the complete branch diff and update the affected
documentation. Regenerate documentation bundles and run their consistency checks.
This check uses repository files and native tools; no external skill is required.
...
This applies whenever a PR is created or its title is edited: the version is the
first token. The final commit subject carrying the version bump follows the same rule.
...
For release preparation, follow `docs/RELEASING.md` using available native tools.
Complete version synchronization, CHANGELOG, affected documentation, tests, and
required review before handoff. Publishing and live activation retain their
existing approval boundaries.

Key routing rules:
- Product ideas, "is this worth building", brainstorming → invoke office-hours
- Bugs, errors, "why is this broken", 500 errors → invoke investigate
- Release preparation, push, create PR → follow `docs/RELEASING.md`
- QA, test the site, find bugs → invoke qa
- Code review, check my diff → invoke review
- Update release docs → perform the release documentation check above
