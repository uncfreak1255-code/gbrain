...
describe('isTrustedDotfile', () => {
  ...
  test('group-writable, even when owned → NOT trusted', () => {
    expect(isTrustedDotfile(fakeStats({ uid: 1000, mode: 0o664 }))).toBe(false);
  });
  ...
  test('no getuid (Windows) → trusted by default', () => {
    ...
    expect(isTrustedDotfile(fakeStats({ uid: 2000, mode: 0o666, symlink: true }))).toBe(false);
  });
});
