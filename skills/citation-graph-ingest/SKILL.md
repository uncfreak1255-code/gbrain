...
(`gbrain sources add` + `gbrain sync`, or `gbrain import`). Confirm scope via
`GBRAIN_SOURCE=<name>` or a `.gbrain-source` dotfile before the graph walk.
Every `link` / `graph-query` call in this pipeline runs under that same source
context — edges must never smear across sources. Do not pass `--source` to
`graph-query`; it is not a supported flag.
...
```bash
GBRAIN_SOURCE=<corpus-source> gbrain graph-query doc-a-example --direction in
GBRAIN_SOURCE=<corpus-source> gbrain graph-query doc-a-example --type overrules --direction in
```
