...
- **Page-delete sweeps** — `gbrain delete <slug>` in a loop, or any script that
  ...
  `gbrain call restore_page '{"slug":"<slug>"}'`) until purged — say so on the
  card, then gate anyway: a sweep that's wrong in bulk is expensive to un-wrong
  in bulk.
- **`purge_deleted_pages` admin operation** — permanently removes soft-deleted
  pages. This is the point of no return for the soft-delete safety net.
