...
> the lookup chain (`search` → `query` → `get`) is the same chain this gate
...
1. **Resolve registry-first, not by the generic phrase.** Use the current
   ...
   not been backfilled yet, run `gbrain reindex --aliases` first. A plausible
   hit means the page exists — STOP, link, don't clone. If the brain also keeps
   an explicit index of named initiatives (e.g. a page under `concepts/`), read
   it before concluding anything is new.
...
4. **When a NEW named thing appears, bake its aliases in the same write.**
   ...
   resolves through alias-aware search. One frontmatter list covers all future
