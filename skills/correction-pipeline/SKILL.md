...
| **FACTS_TABLE_ERROR** | Wrong fact in the gbrain facts table | `recall` → `forget <fact-id>` → `extract_facts` correction |
...
- **SOUL_USER_ERROR:** NEVER edit SOUL.md / USER.md directly — they are
  ...
  underlying answer in the shared bootstrap answer bank, then re-render through
  `skills/soul-audit/SKILL.md`. Do not invent a direct CLI path when the current
  install does not expose one.
...
- **FACTS_TABLE_ERROR:** Expire the wrong row and write the correction with
  ...
  gbrain call extract_facts '{"turn_text":"Correction, YYYY-MM-DD: <correct fact>","entity_hints":["<entity-slug>"],"session_id":"correction-YYYY-MM-DD"}'
  gbrain recall <entity-slug> --grep "<distinct phrase from correct fact>"
  ```
- **HALLUCINATION:** There is no source to fix. Identify the partial signal
  ...
  ```bash
  gbrain call extract_facts '{"turn_text":"Hallucination guard, YYYY-MM-DD. WRONG: <what was said>. RIGHT: <what is true>. Guard: <instruction to prevent recurrence>","entity_hints":["<entity-slug>"],"session_id":"hallucination-guard-YYYY-MM-DD"}'
  gbrain recall <entity-slug> --grep "<distinct guard phrase>"
  ```
- **STALE_DATA:** Update the source page with current truth (BRAIN_ERROR
  flow), and supersede any stale facts rows (`forget` + `extract_facts` with
  the current truth and fresh provenance).
...
## Complement: contradiction scans

This skill is REACTIVE — it fires when the user catches an error. The shipped
evaluation surfaces are the PROACTIVE side of the same discipline when a
repo-local contradiction probe exists: they find intra-brain conflicts before
the user does. If the current install does not expose a contradiction command,
do not claim that scan ran.
...
- Repo-local contradiction probes — PROACTIVE intra-brain conflict detection.
