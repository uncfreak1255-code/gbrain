...
- **Rendered-file safety.** Any recommendation touching a bootstrap-rendered
  file is expressed as an answer-bank or template change through
  `skills/soul-audit/SKILL.md`, never as a direct edit or invented CLI command.
...
# `gbrain models --json` returns the resolved default even when no override is set.
JUDGE=$(gbrain models --json | jq -r '.tiers.utility.resolved')
...
- **Recommending a direct edit to a rendered file.** SOUL.md / USER.md /
  ...
  render. Target the answer bank or template through
  `skills/soul-audit/SKILL.md`, then re-render.
