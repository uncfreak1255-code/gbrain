...
async function handleCliOnly(command: string, args: string[]) {
  ...
  try {
    switch (command) {
      ...
      case 'candidate': {
        const { runCandidate } = await import('./commands/candidate-scorecard.ts');
        await runCandidate(args);
        break;
      }
      case 'think': {
