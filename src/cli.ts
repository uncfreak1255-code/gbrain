...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'corpus') {
    const { runCorpus } = await import('./commands/corpus.ts');
    await runCorpus(null, args);
    return;
  }
  if (command === 'upgrade') {
  ...
  if (command === 'outlook' && (args[0] === 'login' || !args.includes('--write'))) {
    const { runOutlook } = await import('./commands/outlook.ts');
  ...
  if (command === 'seascape' && !args.includes('--write')) {
    const { runSeascape } = await import('./commands/seascape.ts');
  ...
  try {
    switch (command) {
      ...
      case 'operator-brief': {
        const { runOperatorBrief } = await import('./commands/operator-brief.ts');
        runOperatorBrief(args);
        break;
      }
      case 'graph-query': {
...
TOOLS
  ...
  corpus inspect <url-or-path>       Inspect playlist/transcript corpus for brief prep
...
ADMIN
  ...
  serve --http [--port N]            HTTP MCP server with OAuth 2.1
    ...
    --suppress-bootstrap-token       Never print the generated admin token
    --print-admin-token              Print it even when stderr is not a TTY
