...
import { mkdirSync, existsSync, readFileSync, writeFileSync, rmSync, statSync, renameSync, unlinkSync } from 'fs';
...
function startHeartbeat(lockPath: string, ownerToken: string): ReturnType<typeof setInterval> {
  const timer = setInterval(() => {
    try {
      ...
      // Atomic sibling replacement: a contender must see either the previous
      // complete JSON document or the next one, never an in-place truncate /
      // partial-write window that could be mistaken for an abandoned lock.
  ...
  while (Date.now() - startTime < timeoutMs) {
    ...
    if (existsSync(lockDir)) {
      ...
      try {
        ...
        // An unreadable lock may be a live holder between filesystem writes or
        // a just-created lock directory whose owner has not written metadata
        // yet. Never reap on parse/read failure; wait for a complete document
        // or time out with the manual-recovery path.
...
export async function releaseLock(lock: LockHandle): Promise<void> {
  ...
  if (lock.ownerToken) {
    try {
      ...
      // Unreadable is not proof of ownership. Leaving a stale directory is
      // safer than deleting a possibly live replacement lock.
      return;
    }
  }
