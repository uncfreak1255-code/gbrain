...
export function parseAutopilotRuntimeLock(raw: string): AutopilotRuntimeLock {
  ...
  try {
    ...
    if (keys.length !== 2 || keys[0] !== 'managed_worker' || keys[1] !== 'zero_paid_spend') return fallback;
    ...
    return { pid: fallback.pid, zero_paid_spend: record.zero_paid_spend, managed_worker: record.managed_worker };
  } catch {
    return fallback;
  }
}
...
export function classifyAutopilotStatus(input: {
  ...
  return {
    ...
    zero_paid_spend: {
      live_managed_worker: managedWorker === true ? input.runtime?.zero_paid_spend ?? null : null,
