...
const learning_loop_arm: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    return learningLoopCall((mod) => mod.armLearningLoop({
...
const learning_loop_resolve_transcript: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    return learningLoopCall((mod) => mod.resolveAuthoritativeTranscript({ engine: ctx.engine, config: ctx.config, provider: 'codex', provider_session_id: p.provider_session_id as string, source_id: p.source_id as string }));
  },
};
...
const learning_loop_bind_session: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    return learningLoopCall(async (mod) => {
      ...
      await mod.resolveAuthoritativeTranscript({ engine: ctx.engine, config: ctx.config, provider: 'codex', provider_session_id: providerSessionId, source_id: sourceId });
