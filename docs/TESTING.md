...
- `test/autopilot-install.test.ts` — install-target detection, wrapper env source order, and installed autopilot schedule readback used by `gbrain autopilot --status --json` / `gbrain operator-brief`.
...
- `test/eval-longmemeval.test.ts` — LongMemEval harness, hermetic with no `DATABASE_URL` and no API keys: PGLite create + reset over runtime-enumerated `pg_tables`, infrastructure-table preservation across resets, JSONL question parsing, retrieval-only and answer-gen modes via stubbed `ThinkLLMClient`, `--limit` cutoff, `--keyword-only` vs hybrid, default `--expansion=off` behavior, explicit reranker-config seeding (`--reranker-model`, `--reranker-enabled`, `--reranker-timeout-ms`, `--reranker-top-n-in`, `--reranker-top-n-out`), perf gate (p50 < 30ms / p99 < 50ms warm reset+import+search on Apple Silicon), `--help` works without a configured brain, fixture round-trip via `test/fixtures/longmemeval-mini.jsonl`.
...
- `test/nightly-quality-probe.test.ts` + `test/autopilot-nightly-probe-wiring.test.ts` + `test/cycle/nightly-probe-adapters.test.ts` — nightly quality probe path: DI-harnessed pass/fail/rate-limit/embedding-key/budget outcomes, autopilot wiring contract, and argv-shape regression checks for the LongMemEval adapter. Coverage now pins live reranker passthrough from host config into the hermetic nightly LongMemEval benchmark brain.
...
- `test/source-hygiene.test.ts` + `test/doctor-source-path-health.test.ts` + `test/advisor-source-hygiene.test.ts` — read-only source-checkout classifier, fail-closed archive vetoes (including scalar/array/JSON bindings), transactional archive reread, migration-v121 trigger coverage for every exact `source_id` column plus special array/JSON/config/sync-lock references, brain-wide protected-work gate, remote no-filesystem boundary, and consistent Doctor/Planner/Advisor operator decisions. Populated/default missing sources are recovery failures; only fully proven empty non-default sources become adversarial soft-archive candidates.
- `test/e2e/source-hygiene-archive-concurrency.test.ts` — real-Postgres two-connection proof for migration v124's archive/write lock protocol. Existing data and live-sync-lock writers hold `FOR SHARE`, forcing archive to wait and then see the committed reference/lock; an archive holding `FOR UPDATE` forces later data writers and sync-lock refreshes to wait and then reject the archived source. This race cannot be proven by PGLite's single connection.
- `test/storage-export.test.ts` — source-scoped recovery export isolation for overlapping slugs/tags/raw data, complete pagination beyond one batch, `source_page_count === page_count`, deterministic locale-independent manifest hashes, `__proto__` raw-key preservation, traversal confinement, invalid/filter flag rejection, and preserved legacy unscoped behavior.
