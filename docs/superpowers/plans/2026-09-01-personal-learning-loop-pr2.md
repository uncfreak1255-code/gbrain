...
- Every PR 2 canonical learning mutation requires `learning_loop.mode === canary`, a replay-derived active V2 run, and exact equality with its frozen brain, source, slug, corpus binding, and destination-root binding. The only off-mode exception is the narrow disabled-recovery path defined below: it may durably deliver and acknowledge an exact event record already stored in canonical `pending_delivery`, then append the terminal abort. It cannot create authority, activate or reinstate a claim, lift a block, advance a lineage/reversal phase, or begin any new canonical transition.
...
- `disabled_recovery_pending` re-enters only through the frozen run's exact source and destination bindings, in the normal source -> lifecycle -> page -> ledger lock order. While mode is non-canary it may append/read back the exact bytes already present in `pending_delivery`, clear that record with the acknowledgement rename, reconcile the derived database only from that exact acknowledged canonical readback, and append `run_aborted` through the same durable ledger primitive. It rejects absent or different pending bytes, binding drift, a terminal or replaced run, and any request to compute a new event or managed state. Mode remains non-canary throughout. This exception cannot activate, correct, reverse, unblock, reconstruct protected state from the database, or admit another session.
...
7. Implement abort/off snapshot-retry ordering and the narrow `disabled_recovery_pending` exception: under frozen bindings, deliver/read back one exact already-canonical pending event, clear only its delivery marker, append the terminal abort, and reject every new learning transition while mode is off.
...
- Failed abort recovery disables mode and blocks re-arm. A later owner operation may, under exact frozen bindings, durably deliver and acknowledge only the already-canonical pending event, reconcile the derived database from exact canonical readback, and append the terminal abort; tests prove crash -> off -> recovery succeeds while authority creation, activation, block lifting, lineage/reversal advancement, new transitions, and DB-derived protected-state reconstruction remain forbidden.
