...
| Codex prompt and checkpoint hooks | user-global `hooks.json` + a config.toml trust entry under CODEX_HOME (both managed by bootstrap — codex hooks are silently inert without the trust entry) | at prompt, Stop, PreCompact, Interrupt and SessionEnd, machine-wide; `--no-hooks` opts out, `GBRAIN_HOOKS=0` disables |
...
| Codex (no MCP scope flag) | pull protocol + MCP tools + prompt and checkpoint hooks (trust-gated `hooks.json` entries bootstrap manages) | the ability to confine MCP registration to one folder (`codex mcp add` is user-global); use explicit source grants for data scope |
...
- Codex: one managed `[mcp_servers.gbrain]` block with the bearer token
  ...
  codex prompt and checkpoint hooks — the user-global `hooks.json` entries plus their
  `config.toml` trust entries beside the target config (codex hooks are silently
...
- `test/e2e/bootstrap-real-codex.serial.test.ts` — real `codex exec`. It runs the
  ...
  (the pull protocol complements prompt hooks; checkpoint capture is
