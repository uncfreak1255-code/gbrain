...
gbrain autopilot --status --json   # includes installed launchd/systemd/cron/start-script readback
ps aux | grep 'jobs work'

# Full operator receipt: queue, doctor preview, budget, active jobs, source
# drift, and whether the installed autopilot schedule is wired on this host.
gbrain operator-brief
