...
- **Manual Dream runs:** keep autopilot enabled as the normal background mode.
  Pause it only for a manual `gbrain dream` run that writes or spends, verify it
  is stopped, run the bounded invocation, score written slugs with
  `gbrain eval dream-quality`, then restore any temporary model/cap changes and
  restart autopilot. Read-only or dry inspection paths that avoid spend-capable
  phases can run while autopilot is alive; dry synthesize/atom synthesis can
  still spend or cache verdicts and need the pause. Manual Dream and the daemon
  share the cycle lock, workers, write targets, and spend budget; see
...
Use the native repository commands and GitHub workflow described in the release
policy. External skills are optional helpers, not prerequisites. The full
release + contributor process
