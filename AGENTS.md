...
## Sawyer fork / PR boundary

Sawyer's fork is `uncfreak1255-code/gbrain`; upstream is `garrytan/gbrain`.
Default to PRs that Sawyer can merge himself: push branches to `fork` and open
PRs against `uncfreak1255-code/gbrain` unless Sawyer explicitly asks for an
upstream PR to `garrytan/gbrain`.
...
```bash
bun run install:pr-branch-hook
```

The guard fails when `origin/master` is not an ancestor of `HEAD`, which blocks
fork-only or stale work-branch commits from leaking into a PR. If a branch fails,
create a fresh branch from `origin/master` and cherry-pick only the keeper commits.
